  ...
  stage('Get version from chart') {
      steps{
       script{
        ...
        chart = readYaml file: "${WORKSPACE}/${PACKAGE_PATH}/chart/${appname}/Chart.yaml"
      ...
      steps {
        script {
          withCredentials([string(credentialsId: "${OCP_SERVICE_TOKEN}", variable: 'SECRET')]) {
            try {
              sh(script: "cd ${WORKSPACE}/svilrepo/chart/${appname}/ && oc login $OCP_CLUSTER_URL --token=$SECRET --namespace=${NAMESPACE} --insecure-skip-tls-verify && helm upgrade --install ${appname} .", returnStdout: true)
            }
            catch (Exception e) {
              echo e.getMessage()
