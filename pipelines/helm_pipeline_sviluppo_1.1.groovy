...
*/
  ...
  stage('Docker File') {
   steps{
    sh "cd  ${WORKSPACE}/${PACKAGE_PATH} && docker build  --build-arg HTTP_PROXY=http://10.204.68.5:8080 --build-arg HTTPS_PROXY=http://10.204.68.5:8080 --tag ${IMAGE_NAME} -f ${DOCKER_FILE} ."
    }
   }
  ...
  stage('Docker TAG') {
   steps {
    script {
     try{
      def updateResult = sh(script: "HTTP_PROXY=http://10.204.68.5:8080 HTTPS_PROXY=http://10.204.68.5:8080 docker tag ${IMAGE_NAME}:latest ${DOCKER_REGISTRY}/${IMAGE_NAME}:${version}", returnStdout: true)
      echo updateResult;
        }catch(e){
    ...
    script {
     try{
      withCredentials([usernamePassword(credentialsId: "$NEXUS_CREDENTIALS", usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD')]){
       def updateResult = sh(script: "HTTP_PROXY=http://10.204.68.5:8080 HTTPS_PROXY=http://10.204.68.5:8080 docker login ${DOCKER_REGISTRY} -u $USERNAME -p $PASSWORD ", returnStdout: true)
       echo updateResult;
          }
         }catch(e){
  ...
  stage('Docker Push') {
   steps {
    script {
     try{
      def updateResult = sh(script: "HTTP_PROXY=http://10.204.68.5:8080 HTTPS_PROXY=http://10.204.68.5:8080 docker push ${DOCKER_REGISTRY}/${IMAGE_NAME}:${version}", returnStdout: true)
      echo updateResult;
        }catch(e){
        ...
        script {
          withCredentials([string(credentialsId: "${OCP_SERVICE_TOKEN}", variable: 'SECRET')]) {
            try {
              ...
              sh 'cd ${WORKSPACE}/svilrepo/chart/${IMAGE_NAME}/ && oc login $OCP_CLUSTER_URL --token=$SECRET --namespace=${NAMESPACE} --insecure-skip-tls-verify && helm upgrade --install ${IMAGE_NAME} .'
            }
            catch (Exception e) {
              echo e.getMessage()
