        ...
        script {
          withCredentials([string(credentialsId: "$OCP_SERVICE_TOKEN", variable: 'SECRET')]) {
            try {
              ...
              sh 'cd ${WORKSPACE}/svilrepo/helm/${APP}/ && oc login $OCP_CLUSTER_URL --token=$SECRET --namespace=${NAMESPACE} --insecure-skip-tls-verify && helm upgrade --install ${APP} -f ${WORKSPACE}/confrepo/${AMBIENTE}/${TEMPLATE_VERSION_TAG}/${AMBIENTE}_${APP}.values .'
            }
            catch (Exception e) {
              echo e.getMessage()
